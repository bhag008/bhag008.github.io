...
| 7 | 古龍神ヴォルカドラゴン | 進化の秘伝書（単色竜、元プールで組める最強構成）、自身のレジェンダリーカードを使用 | 85G/15G |
| 8 | 深淵の始祖ネクロズアリオン | 進化の秘伝書（単色悪魔、元プールで組める最強構成）、自身のレジェンダリーカードを使用 | 80G/14G |

**レベル7・8の位置づけ**: プレイヤー用デッキ調整で作った「竜デッキv5」「デーモンデッキ(最終形)」をベースに、進化ペイオフ・進化元・トークン生成・進化サーチ（`ev_r3`）を全て2枚ずつ採用した「元プールで作れる最強構成」。簡易プレイヤー(`veteran`デッキ)基準でのCPU勝率は竜89.8%・悪魔87.0%（N=400）で、妖精女王(85.6〜90%)と同格、魔王ザガレス・紅蓮の竜王(95〜98%)よりはやや易しい。レベル数字は追加順であり強さ順ではない（竜の方が僅かに測定値が高いが誤差範囲）。UIの星表示（`opponentSelect.js`の`STARS`）もレベル7・8分を追加済み（🐉／👹サフィックス）。
...
5. **不要な集中攻撃への参加を回避**（重要）: 上記4の「合計攻撃力で倒しきる」判定は、自分を含めた全未行動アタッカーの合計で行っていたため、**自分抜きの他アタッカーだけで倒しきれる場合でも無条件で突撃していた**（例: 3/5挑発に対し1/1を3体突撃させた後、本来は単独で相打ちを取れる5/2だけで十分だった、という無駄死に）。`othersAtk`（自分を除いた未行動アタッカーの合計攻撃力）が対象のHPを下回る場合のみ、つまり自分の参加が本当に必要な場合のみ攻撃するよう修正した。ユーザー報告により発覚・修正（2026年時点）。
...
**5の修正後の副作用**: エンジンレベルでは意図通り動作する（無駄死に解消・温存したユニットは次のパスで空いた顔面を攻撃するなど厳密に旧ロジックより得）ことを確認済みだが、`veteran`デッキ基準のCPU対人勝率を旧ロジックとクリーンにA/B比較したところ、挑発を多く採用するCPUデッキ（紅蓮の竜王・魔王ザガレス・妖精女王・ネクロズアリオン）で3〜6pt程度勝率が下がる副作用が観測された（多ターンにまたがる複雑な相互作用が原因と推測されるが未解明）。ユーザー判断で報酬額の再調整は見送り、修正のみ採用した。
